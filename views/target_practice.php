...
$no_of_same_distractor_per_group = 1; // eg., x 2,2 distractors

$no_of_trial_for_group = 1; // x trials in total
...
if ($request_method == 'POST' && isset ( $_POST ['participant_name'] )) {
	...
	$gender = $_POST ['gender'];
	$participant_id = $participantMgr->insert ( $participant_name,$age,$gender ); // to db
...
if (isset ( $_GET ['status'] ) && isset ( $_GET ['trial_id'] )) {
	...
	// print "<br> stat: $attempt_status";
	// print "<br> tid: $trial_id";
	...
	if ($attempt_status == false) {
		// print "<br> status: $attempt_status";
		...
		// print "<br> arranged_ids from db $arranged_ids";
...
//$isMaxLimit = $trialMgr->isMaxLimit ( $no_of_trial_for_group, $participant_id );
...
if ($isMaxLimit == false && ($initial == true || isset ( $attempt_status ) && $attempt_status == true)) {
	...
	//print "<br>full $full_icon_count";
	//print "<br>colour $colour_n_name_count";
	//print "<br>shape $shape_n_name_count";
	//print "<br>only $name_only_count";
	
	$flag = false;
	while ( $flag == false ) { //compare if flag is false
								//if false, enter condition
...
// 																								
																									print "<script type=\"text/javascript\">location.href = 'start_experiment.php?participant_id=$participant_id';</script>";
																								}
						...
						<tr>
							...
							<td>
								...
								<button onclick="location.href = 'menu_practice.php'" type="button"
